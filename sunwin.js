// ============================================================
// PHẦN 1/5: IMPORTS + CONFIG + PATTERN DATABASE
// ============================================================
...
// ============================================================
// PHẦN 2/5: 14 THUẬT TOÁN AI + HELPERS
// ============================================================
...
function algo14_lstmDatasetHybrid(history) {
    ...
    const matches = dataset.queryPattern(recent, 10);
    if (matches.length < 5) return lstmResult.confidence > 0.7 ? lstmResult.prediction : null;
...
if (dataset.totalSamples > 0) {
    ...
    setTimeout(() => {
        try { lstmModel.trainOnDataset(trainingSubset, 2); }
        catch (e) { console.error('❌ LSTM training lỗi:', e.message); }
    }, 100);
} else {
    console.log('⚠️ Không có dataset → LSTM bị vô hiệu hóa');
}
// ============================================================
// PHẦN 4/5: AI CORE CLASS
// ============================================================
class AdvancedDeepLearningAI {
    ...
    predict() {
        ...
        if (this.history.length < minHistory) return { prediction: 'tài', confidence: 0.5, rawPrediction: 'T', algorithms: 0 };
        const predictions = [];
        this.recentPredictions = {};
        ALGORITHMS.forEach(algo => {
            try {
...
// ============================================================
// PHẦN 5/5: FASTIFY SERVER + API ENDPOINTS + POLLING
// ============================================================
...
function buildPredictionPayload() {
    ...
    if (!lastResult) {
        return {
            ...
            ai_version: "12.0 - @cskhgiabao API History Version",
...
app.get("/api/sunwin/tx/only", async (request, reply) => {
    try {
        ...
        const lastResult = valid.length ? valid[0] : null;
        const currentPrediction = ai.predict();
        if (!lastResult) return { success: false, du_doan: null, phien: null };
        return {
            success: true,
...
app.get("/api/sunwin/tx/history", async (request, reply) => {
    try {
        const valid = rikResults.filter((r) => r.dice?.length === 3);
        if (!valid.length) return { success: false, history: [] };
        return {
            success: true,
            total: valid.length,
...
app.get("/api/taixiu/ai-stats", async () => {
    ...
    } catch (e) { return { error: "Lỗi hệ thống" }; }
});
...
app.get("/", async () => {
    return {
        ...
        version: "12.0 - API History Version",
